...
void Game::update(float elapsed) {
	...
	for (auto &p : players) {
		...
		while(p.next_move_timer < 0) {
			...
			//wrap around
			if(new_head_pos.x >= map.width) new_head_pos.x = 0;
			if(new_head_pos.x < 0) new_head_pos.x = map.width-1;
			if(new_head_pos.y >= map.height) new_head_pos.y = 0;
			if(new_head_pos.y < 0) new_head_pos.y = map.height-1;
