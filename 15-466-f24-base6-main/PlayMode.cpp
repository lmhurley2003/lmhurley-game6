...
PlayMode::PlayMode(Client &client_) : client(client_), scene(*snake_scene) {
	...
	for (uint32_t y = 0; y < game.map.height; y++) {
		for(uint32_t x = 0; x < game.width; x++) {
			...
			drawable.transform.position = glm::vec3(float(x)+0.5f, float(y)+0.5f, -0.05f);
			if(block == G) {
				...
				assert(false + "invalid block enum in map!");
			}
		}
		scene.drawables.emplace_back(drawable);
	}

	non_entity_end = scene.drawables.end();

	if (scene.cameras.size() != 1) throw std::runtime_error("Expecting scene to have exactly one camera, but it has " + std::to_string(scene.cameras.size()));
	camera = &scene.cameras.front();
	camera.position.x = game.map.width / 2.0f;
	camera.position.y = -game.map.height * 1.25f;
}
...
void PlayMode::update(float elapsed) {
	...
	//refresh lists
	auto start = non_entity_end;
	//add player blocks
	for (auto &p : game.players) {
		for(auto block = p.block_positions.start(); block != p.block_positions.end(); block++) {
			glm::ivec3 b = *block;
			Scene::Drawable drawable;
			drawable.transform.position = glm::vec3(float(b.x)+0.5f, float(b.y)+0.5f, float(b.z));
			if(std::next(block) == p.block_positions.end()) {
				drawable.pipeline = snakeHeadPrefab;
				if(p.move_dir == left) {
					drawable.transform.rotation *= glm::angleAxis(90.0f, glm::vec3(0.0f, 0.0f, 1.0f));
				} else if(p.move_dir == down) {
					drawable.transform.rotation *= glm::angleAxis(180.0f, glm::vec3(0.0f, 0.0f, 1.0f));
				} else if(p.move_dir == right) {
					drawable.transform.rotation *= glm::angleAxis(270.0f, glm::vec3(0.0f, 0.0f, 1.0f));
				}
			} else {
				drawable.pipeline = snakeCubePrefab;
			}
	
			if(start != scene.drawables.end()) {
				*start = drawable;
				start = std::next(start);
			} else {
				scene.drawables.emplace_back(drawable);
				start = scene.drawables.end();
			}
		}
	}

	//add apples
	for(auto &_apple = apples.start(); _apple != apples.end(); _apple++) {
		Apple a = *_apple;
		Scene::Drawable drawable;
		drawable.transform.position = glm::vec3(float(a.x)+0.5f, float(a.y)+0.5f, float(a.z)+0.5f);
		drawable.pipeline = applePrefab;
	...
	while(start != scene.drawables.end()) {
		auto it_n = std::next(start);
		scene.drawables.erase(start);
		start = it_n;
	}
